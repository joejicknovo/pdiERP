...
BEGIN
	...
	SET @sql = 'SELECT * FROM dbo.bid_v WHERE 1 = 1';
	SELECT @count = COUNT(*) FROM dbo.bid_v; 

	IF @search_param IS NOT NULL  
	BEGIN
		SET @sql = @sql + 'AND bid_code LIKE ''%' + @search_param + '%'' ';
		SET @sql = @sql + 'OR UPPER(client_name) LIKE UPPER(''%' + @search_param + '%'') ';
		SET @sql = @sql + 'OR UPPER(project_name) LIKE UPPER(''%' + @search_param + '%'') ';
		SET @sql = @sql + 'OR bid_date LIKE ''%' + @search_param + '%'' ';
		SET @sql = @sql + 'OR submission_date LIKE ''%' + @search_param + '%'' ';
		SET @sql = @sql + 'OR selling_cost LIKE ''%' + @search_param + '%'' ';
		SET @count = 1;
	END
	ELSE
	BEGIN
		IF @bid_id IS NOT NULL  
		BEGIN
			SET @sql = @sql + 'AND bid_id =  ' + CAST(@bid_id AS NVARCHAR(10)) + ' ';
			SET @count = 1;
		END
	END
	
	SET @sql = @sql + 'ORDER BY bid_code';
