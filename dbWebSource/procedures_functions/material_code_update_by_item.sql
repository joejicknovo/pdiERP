...
CREATE PROCEDURE [dbo].[material_code_update_by_item]
(
	@tt    material_code_update_by_item_tt READONLY
	, @user_id int
)
AS
...
BEGIN
	...
	(
	  ...
	  material_item_code NVARCHAR(15)
	);

	DECLARE @material_item_id INT;
	DECLARE @m_code NVARCHAR(15);

	INSERT INTO @ttt SELECT * FROM @tt WHERE material_item_id IS NOT NULL;

	WHILE EXISTS(SELECT * FROM @ttt)
	BEGIN
		SELECT TOP 1 @material_item_id = material_item_id FROM @ttt;
		SELECT TOP 1 @m_code = material_item_code FROM @ttt;
	
		INSERT INTO @tmp_material (material_id, material_code)
		SELECT material_id, STUFF(material_code, 3, 1, @m_code) AS material_code
		FROM material
		...
		End
		DELETE @ttt WHERE material_item_id = @material_item_id;
