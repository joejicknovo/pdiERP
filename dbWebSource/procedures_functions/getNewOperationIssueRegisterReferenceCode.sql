...
CREATE FUNCTION [dbo].[getNewOperationIssueRegisterReferenceCode](
) 
RETURNS NVARCHAR(20) 
AS
BEGIN
	...
	SET @last_reference_number = (SELECT MAX(reference_no) FROM dbo.operation_issue_register)
	SET @t_last_reference_number = (SELECT SUBSTRING(@last_reference_number, CHARINDEX('-', @last_reference_number) + 1 , 250))
    
	SET @reference_number = 'OIR'
